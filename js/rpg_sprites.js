...
Sprite_Button.prototype.updatePressAnimation = function() {
        if (this._touching) {
            // 按下时缩小到 0.9 倍
            this.scale.x = 0.9;
            this.scale.y = 0.9;
...
(function() {
    ...
    Sprite_Picture.prototype.initialize = function(pictureId) {
        _Sprite_Picture_initialize.call(this, pictureId);        
            this._clickHandler = null; // 存储外部传入的函数
        
    };

    // --- 2. 定义设置回调的方法 (类似 Sprite_Button) ---
    Sprite_Picture.prototype.setClickHandler = function(method) {
        //$gameSwitches.value(491);
        this._clickHandler = method;
    ...
    Sprite_Picture.prototype.update = function() {
        ...
        if (TouchInput.isTriggered()) {
            ...
            var allSprites = container.children;
            for (var i = allSprites.length - 1; i >= 0; i--) {
                var sprite = allSprites[i];

                // 基础判定：显示、有图、没被销毁
                if (sprite.visible && sprite.bitmap && sprite.parent) {
                // 获取鼠标相对于图片的坐标，判定是否在矩形内
                if (sprite.getBounds().contains(TouchInput.x, TouchInput.y)) {
                
                    // --- 找到了最上层图片！ ---
                    sprite.processTouch();
                    // --- 关键：执行完后立即 break，停止向下查找 ---
                    break; 
            }
        }
    }
    ...
    Sprite_Picture.prototype.processTouch = function() {
        ...
        if (this.isClickInRect(tx, ty)) {
            this.callHandler();
            TouchInput.clear();
        }
    };

    Sprite_Picture.prototype.isClickInRect = function(tx, ty) {
        var px = this.x;
        var py = this.y;
        return tx >= px && ty >= py && tx < px + this.width && ty < py + this.height;
    };

    Sprite_Picture.prototype.callHandler = function() {
        if (this._clickHandler) {
            this._clickHandler(); // 执行外部绑定的逻辑
        }
    };
})();
